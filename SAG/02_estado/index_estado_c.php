...
resguardo_partidas.id_resguardo_partidas,
resguardo_partidas.unidades,
determinantes.id_determinantes,
determinantes.clave_determinante,
determinantes.descripcion,
tm.matricula AS 1matricula,
tm.rfc AS 1rfc,
tm.nombre AS 1nombre,
tv.matricula AS 2matricula,
tv.rfc AS 2rfc,
tv.nombre AS 2nombre,
resguardo_partidas.id_estado_fisico,
estado_fisico.clave_estado_fisico
FROM
resguardo
INNER JOIN periodo ON periodo.id_periodo = resguardo.id_periodo
INNER JOIN dependencia ON dependencia.id_dependencia = resguardo.id_dependencia
INNER JOIN area ON area.id_area = resguardo.id_area
INNER JOIN consecutivo ON consecutivo.id_consecutivo = resguardo.id_consecutivo
INNER JOIN resguardo_partidas ON resguardo.id_resguardo = resguardo_partidas.id_resguardo
INNER JOIN determinantes ON determinantes.id_determinantes = resguardo_partidas.id_determinantes
INNER JOIN empleado AS tm ON tm.id_empleado = resguardo.id_empleado_tm
INNER JOIN empleado AS tv ON tv.id_empleado = resguardo.id_empleado_tv
INNER JOIN estado_fisico ON resguardo_partidas.id_estado_fisico = estado_fisico.id_estado_fisico

WHERE resguardo.id_dependencia = %s and resguardo.id_periodo = %s and
resguardo_partidas.id_estado_fisico = %s

ORDER BY
	area.clave_area ASC,
	consecutivo.clave_conse ASC,
	determinantes.clave_determinante ASC", 
GetSQLValueString($colname_Area3, "int"),
GetSQLValueString($colname2_Area3, "int"),
...
<blockquote>
  <p>
  <h1>Activos por Area Asignación  Estado C</h1> 
  Orden del Reporte  Area, Consecutivo, Clave Determinante
  </p>
</blockquote>
&nbsp;
...
<table width="260%" border="0" align="center" class="table table-hover">
    ...
    <tr class="info">
      ...
      <td colspan="3"><h4>Determinante</h4></td>
...
<td><h6><?php echo $row_Area3['clave_estado_fisico']; ?></h6></td>
