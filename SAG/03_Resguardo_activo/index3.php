...
$query_Area3 = sprintf("SELECT resguardo.id_resguardo, 	resguardo.id_periodo, 	resguardo.id_dependencia, 	resguardo.id_area, 	resguardo.id_consecutivo, 	periodo.semestre, 	dependencia.clave_dependencia, 	dependencia.depen_descripcion, 	area.clave_area, 	area.des_area, 	consecutivo.clave_conse, 	consecutivo.descripcion_consecutivo, 	tm.matricula AS 1matricula, 	tm.rfc AS 1rfc, 	tm.nombre AS 1nombre, 	tv.matricula AS 2matricula, 	tv.rfc AS 2rfc, 	tv.nombre AS 2nombre FROM resguardo INNER JOIN periodo ON periodo.id_periodo = resguardo.id_periodo INNER JOIN dependencia ON dependencia.id_dependencia = resguardo.id_dependencia INNER JOIN area ON area.id_area = resguardo.id_area INNER JOIN consecutivo ON consecutivo.id_consecutivo = resguardo.id_consecutivo INNER JOIN empleado AS tm ON tm.id_empleado = resguardo.id_empleado_tm INNER JOIN empleado AS tv ON tv.id_empleado = resguardo.id_empleado_tv WHERE resguardo.id_dependencia = %s and resguardo.id_periodo = %s GROUP BY resguardo.id_consecutivo, resguardo.id_area, resguardo.id_resguardo ORDER BY consecutivo.clave_conse ASC, area.clave_area ASC, resguardo.id_resguardo ASC", GetSQLValueString($colname_Area3, "int"),GetSQLValueString($colname2_Area3, "int"));
...
<title>Resguardos por Consecutivo</title>
...
<blockquote>
  <p>
  <h1>Resguardos por Consecutivo</h1> 
  Orden del Reporte  Consecutivo,  Area, Número Resguardo</blockquote>
&nbsp;
...
<tr>
  <td><h6>Total Resguardos:</h6></td>
  <td><h6><?PHP echo $nRes ?></h6></td>
<td><h6>&nbsp;</h6></td>
<td><h6></h6></td>
<td><h6>&nbsp;</h6></td>
<td><h6></h6></td>
