...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO vale_salida (id_vale_salida, id_vale_salida_tipo, id_periodo, id_dependencia, id_empleado_tm, id_empleado_tv, fecha, numero_vale, fecha_vale_traspaso, numero_factura, observaciones, id_subdependencia ) 
...
mysql_select_db($database_SAG, $SAG);
$query_vales_salida_tipo = "SELECT * FROM vale_salida_tipo ORDER BY clave_vale_salida_tipo ASC";
$vales_salida_tipo = mysql_query($query_vales_salida_tipo, $SAG) or die(mysql_error());
$row_vales_salida_tipo = mysql_fetch_assoc($vales_salida_tipo);
$totalRows_vales_salida_tipo = mysql_num_rows($vales_salida_tipo);
?>
...
<title>Nuevo Vale de Salida</title>
...
<blockquote>
  <p>
  <h1>Nuevo Vale de Salida</h1>
  </p>
</blockquote>
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" id="form1">
<input type="hidden" name="id_vale_salida" value="" />
  <table align="center" width="100%">
    ...
    <tr valign="baseline">
      ...
      <td width="121"><p align="left"><select name="id_vale_salida_tipo" id="id_vale_salida_tipo">
        <option value="0"></option>
        <?php
do {  
?>
        <option value="<?php echo $row_vales_salida_tipo['id_vale_salida_tipo']?>"><?php echo $row_vales_salida_tipo['descripcion_vale_salida_tipo']?></option>
        <?php
} while ($row_vales_salida_tipo = mysql_fetch_assoc($vales_salida_tipo));
  $rows = mysql_num_rows($vales_salida_tipo);
  if($rows > 0) {
      mysql_data_seek($vales_salida_tipo, 0);
	  $row_vales_salida_tipo = mysql_fetch_assoc($vales_salida_tipo);
  }
?>
      </select></p></td>
...
mysql_free_result($vales_salida_tipo);
?>
