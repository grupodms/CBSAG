...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE vale_salida_partidas SET id_vale_salida=%s, id_estado_fisico=%s, num_serie=%s, salida_vale=%s, numero_inventario=%s, observaciones=%s, num_seriea=%s, num_serieb=%s, alta=%s, baja=%s , id_determinantes =%s, unidades =%s WHERE id_vale_salida_partidas =%s",
 ...
 GetSQLValueString($_POST['id_estado_fisico'], "int"),
                       ...
                       GetSQLValueString($_POST['salida_vale'], "date"),
...
// echo  $updateSQL; 
 ...
 $updateGoTo .= "?id_vale_salida=".$_POST['id_vale_salida'];
...
$colname_vale_salida = "-1";
if (isset($_GET['id_vale_salida'])) {
  $colname_vale_salida = $_GET['id_vale_salida'];
}
mysql_select_db($database_SAG, $SAG);
$query_vale_salida = sprintf("SELECT vale_salida.id_vale_salida,  vale_salida.fecha,  vale_salida.id_periodo,  periodo.semestre,  vale_salida.id_dependencia,  dependencia.clave_dependencia,  dependencia.depen_descripcion,  vale_salida.id_empleado_tm,  empleado_tm.matricula as tm_matricula,  empleado_tm.rfc as tm_rfc,  empleado_tm.curp as tm_curp,  empleado_tm.nombre as tm_nombre,  empleado_tm.puesto as tm_puesto,  empleado_tm.adcripcion as tm_adcripcion,   vale_salida.id_empleado_tv,  empleado_tv.matricula as tv_matricula,  empleado_tv.rfc as tv_rfc,  empleado_tv.curp as tv_curp,  empleado_tv.nombre as tv_nombre,  empleado_tv.puesto as tv_puesto,  empleado_tv.adcripcion as tv_adcripcion FROM vale_salida  INNER JOIN dependencia ON vale_salida.id_dependencia = dependencia.id_dependencia  INNER JOIN periodo ON vale_salida.id_periodo = periodo.id_periodo INNER JOIN empleado AS empleado_tm ON vale_salida.id_empleado_tm = empleado_tm.id_empleado INNER JOIN empleado AS empleado_tv ON vale_salida.id_empleado_tv = empleado_tv.id_empleado WHERE id_vale_salida = %s", GetSQLValueString($colname_vale_salida, "int"));
...
$colname_vale_salida_detalle = "-1";
if (isset($_GET['id_vale_salida_partidas'])) {
  $colname_vale_salida_detalle = $_GET['id_vale_salida_partidas'];
}


mysql_select_db($database_SAG, $SAG);
$query_vale_salida_detalle = sprintf("SELECT * FROM vale_salida_partidas WHERE id_vale_salida_partidas = %s", GetSQLValueString($colname_vale_salida_detalle, "int"));
$vale_salida_detalle = mysql_query($query_vale_salida_detalle, $SAG) or die(mysql_error());
$row_vale_salida_detalle = mysql_fetch_assoc($vale_salida_detalle);
$totalRows_vale_salida_detalle = mysql_num_rows($vale_salida_detalle);
...
<title>Editar Determinantes de Vale de Salida</title>
...
<blockquote>
  <p>
  <h1>Editar Determinantes  de Vale de Salida.</h1>
  </p>
</blockquote>
<?php
...
<table width="500" border="0" class="btn-group-xs">
  ...
  <tr>
    ...
    <td width="210" align="left">(<?php echo $row_vale_salida['clave_area']; ?>)</td>
    <td width="300" align="left"><?php echo $row_vale_salida['des_area']; ?></td>
  </tr>
  <tr>
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" id="form1">
  <table align="center">
    ...
    <tr valign="baseline">
      ...
      <td colspan="3"><input type="text" name="id_vale_salida" value="<?php echo $row_vale_salida['id_vale_salida']; ?>" size="32" 
                 readonly="readonly" 
                 id = "date1picker"
                 class="alert-success"  
           />
      <input type="text" name="id_vale_salida_partidas" value="<?php echo $row_vale_salida_detalle['id_vale_salida_partidas']; ?>" size="32" 
                 readonly="readonly" 
                 id = "date1picker2"
                 class="alert-success"  
           /></td>
    ...
    <tr valign="baseline">
      ...
      <input name="id_determinantes" type="text" class="alert-success" id="id_determinantes" value="<?php echo $row_vale_salida_detalle['id_determinantes']; ?>" size="5" readonly="readonly" /></td>
      <td width="48"><label for="clave_determinante"></label>
      <input name="clave_determinante" type="text" class="alert-success" id="clave_determinante" value="<?php echo $row_vale_salida_detalle['clave_determinante']; ?>" size="10" readonly="readonly" /></td>
...
do {  
?>
          <option value="<?php echo $row_determinante_2['id_determinantes']?>"<?php if (!(strcmp($row_determinante_2['id_determinantes'], $row_vale_salida_detalle['id_determinantes']))) {echo "selected=\"selected\"";} ?>><?php echo $row_determinante_2['clave_determinante']?></option>
          <?php
...
?>
    ...
    <tr valign="baseline">
      ...
      <td colspan="3"><textarea name="descripcion" cols="45" rows="5" readonly="readonly" class="alert-success" id="descripcion"><?php echo $row_vale_salida_detalle['descripcion']; ?></textarea></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Unidades:</td>
      <td colspan="3"><input name="unidades" type="text" value="<?php echo $row_vale_salida_detalle['unidades']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
...
?>
    ...
    <tr valign="baseline">
      ...
      <td colspan="3"><input name="num_serie" type="text" id="num_serie" value="<?php echo $row_vale_salida_detalle['num_serie']; ?>" size="32" /></td>
    </tr>

    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Numero de vale entrada:</td>
      <td colspan="3"><input name="salida_vale" type="text" class="alert-success" id="salida_vale" value="<?php echo $row_vale_salida_detalle['salida_vale']; ?>" size="32" readonly="readonly" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Numero inventario:</td>
      <td colspan="3"><input name="numero_inventario" type="text" id="numero_inventario" value="<?php echo $row_vale_salida_detalle['numero_inventario']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td align="right" valign="top" nowrap="nowrap">observaciones:</td>
      <td colspan="3"><label for="observaciones"></label>
      <textarea name="observaciones" id="observaciones" cols="45" rows="5"><?php echo $row_vale_salida_detalle['observaciones']; ?></textarea></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Num. serie 2:</td>
      <td colspan="3"><input name="num_seriea" type="text" id="num_seriea" value="<?php echo $row_vale_salida_detalle['num_seriea']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Num. serie 3:</td>
      <td colspan="3"><input name="num_serieb" type="text" id="num_seriea2" value="<?php echo $row_vale_salida_detalle['num_serieb']; ?>" size="32" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">alta:</td>
      <td colspan="3"><input <?php if (!(strcmp($row_vale_salida_detalle['alta'],1))) {echo "checked=\"checked\"";} ?> type="checkbox" name="alta" id="alta" />
      <label for="alta"></label></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">baja:</td>
      <td colspan="3"><input <?php if (!(strcmp($row_vale_salida_detalle['baja'],1))) {echo "checked=\"checked\"";} ?> type="checkbox" name="baja" id="baja" /></td>
    </tr>                
...
<a href="determinantes_index.php?id_vale_salida=<?php echo $row_vale_salida['id_vale_salida']; ?>">
...
mysql_free_result($vale_salida_detalle);
